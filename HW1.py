...
def iterSumNodes(root):
    ...
    while queue:
        ...
        sum += currentNum.value
        if currentNum.subnodes:
            for i in currentNum.subnodes:
                queue.append(i)
    return sum
#################################
# Problem 5
...
#################################
# Objectives:
# (1) Write a yieldTwice function, which takes any iterable (like a list,
# generator, etc) and yields each element of the iterable twice.
...
# yields the returned value for any valid inputs while ignoring any that raise a ValueError.
